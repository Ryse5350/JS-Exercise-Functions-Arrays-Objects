...
function makeSmartPerson(name) {
  ...
  let speak = () => {return `Hello, my name is ${smartPerson.name}`}
  let smartPerson = {
    name: name,
    sum,
    speak,
...
function sortCarInventory(inventory) {
  ...
  return inventory.sort(function(a, b){
    let carA=a.car_model.toLowerCase(), carB=b.car_model.toLowerCase()
    if (carA < carB)
        return -1 
    if (carA > carB)
        return 1
